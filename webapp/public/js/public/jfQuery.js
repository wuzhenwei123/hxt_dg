...
function dataCh(myData) {
    ...
    for (var i = 0; i < obj.length; i++) {
    	...
    	str += '</div>';
//    	if(!obj[i].consignee||!obj[i].consignee_address||!obj[i].consignee_phone||!obj[i].zip_code){
//    		infoFlag = true;
//    	}
    	if(!obj[i].consignee_phone){
    		infoFlag = true;
    	}
    	//缴费号列表
        ...
        for (var j = 0; j < ammeters.length; j++) {
        	...
        	str += '<td width="200px">'+eval(ammeters[j]).ammeter_name+' </td>'; 
        	...
        	str += '<td>'+pay_statu+'</td>';
        	if (ammeters[j].pay_status == '0') {
        		str += '<td class="yjBtnTdBox"><a href="javascript:updateAmmeter('+eval(ammeters[j]).a_id+',1)" id="operat_'+eval(ammeters[j]).a_id+'">恢复</a><a href="javascript:bind2(\''+obj[i].s_id+'\',\''+eval(ammeters[j]).ammeter_no+'\');">详情</a><a href="javascript:delAmmeter(\''+obj[i].s_id+'\',\''+eval(ammeters[j]).ammeter_no+'\')">删除</a></td>';
        	}
        	if (ammeters[j].pay_status == '1') {
        		str += '<td class="yjBtnTdBox"><a href="javascript:updateAmmeter('+eval(ammeters[j]).a_id+',0)" id="operat_'+eval(ammeters[j]).a_id+'">解绑</a><a href="javascript:bind2(\''+obj[i].s_id+'\',\''+eval(ammeters[j]).ammeter_no+'\');">详情</a><a href="javascript:delAmmeter(\''+obj[i].s_id+'\',\''+eval(ammeters[j]).ammeter_no+'\')">删除</a></td>';
        	}
        	str += '</tr>'
        }
        str += '<tr><td>'+(++num1)+'</td><td>抄电表</td><td><div class="lines pItem" style="width:126px;"><input id="ammeterNum_bind_'+obj[i].s_id+'" type="text" placeholder="点击输入缴费号" class="pInput"></div></td><td></td><td></td><td></td><td class="yjBtnTdBox"><a href="javascript:bind(\''+obj[i].s_id+'\',\''+obj[i].c_id+'\');">绑定</a></td></tr>';
        str += '</table>';
        str += '</div>';
        //小计
...
function showDetail5(){
	...
	html += '<div class="lineP searchItem  sItem">';
//	html += '<label class="lines codeTitle" onmouseover="showlabel()" onmouseout="shutlabel()">分组名称：</label>';
	html += '<label class="lines codeTitle fz_title" style="color:#01a796;text-decoration:underline;">单位名称：';
//	html += '<div class="fzRem">';
//	html += '<div class="fz_title_a">为何要设置分组？</div>';
//	html += '<div class="fz_rem">';
//	html += '<p>1、当您有多个缴费号，并且希望交费发票递送到不同地址时，您可以将同一收件地址的缴费号分成一个组；</p>';
...
function updateHSubCompany() {
	...
	var url = $("#projectPath").val() + '/ammetermanager/updateHSubCompany';
	$.ajax({
		type: "POST",
		url: url,
		data: data,
...
//删除
function delAmmeter(s_id,ammeterNo){
