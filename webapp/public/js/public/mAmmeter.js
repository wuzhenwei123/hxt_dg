...
function titleStr(cid,cname,count,fee,yuFee,yuNo){
	...
	str += '<div class="pTitle"><table><tr><td width="72%">您绑定了<span id="ammetercount">'+count+'</span>个缴费号，缴费金额共计：'+fee+'元';
	if(yuFee>0&&yuNo){
		str += '<br>您的缴费号：'+yuNo;
		str += '目前的欠费是预收费，共计：'+yuFee+'元';
	}
	str += '</td><td style="vertical-align:text-top;"><a id="jfHcm" class="jfHcm" href="javascript:showbank();">下载企业网银缴费手册</a></td><tr></table></div>';
	str += '<div class="jyBtnBox">';
	str += '      <div class="jyRemBtnItem"><a id="jfHcm" class="fl jyBtn" href="javascript:beforePay(1,null);">全部用网银交费</a><div class="jyRem">全部缴费号用网银一次性简便缴费</div></div>';
	str += '<div class="jyRemBtnItem"><a class="fl jyBtn" href="javascript:payInfo('+cid+',\''+cname+'\')">全部转发给财务交费</a><div class="jyRem">在有分组的情况下，所有分组的缴费号都交由财务用网银一次性缴费</div></div>';
	str += ' <div class="jyRemBtnItem"><a class="fl jyBtn" href="javascript:showDetail4();">新增电表分组</a><div class="jyRem">如果您需要管理多个不同项目的电费，请根据项目情况进行分组</div></div>';
	str += ' <div class="jyRemBtnItem"><a class="fl jyBtn" href="javascript:exportExcle();">导出报表</a><div class="jyRem">将已绑定的缴费号欠费信息导出Excel表</div></div>';
	str += '  </div>';
	str += '</div>';*/
...
function dataCh(myData) {
	...
	var flagStr = '';
//	str += '<div class="commBody jyItem" style="font-size:14px;"><input type="checkbox" style="margin-left:26px;" id="selectAll" onchange="selectAll()"><label style="margin-left:5px;" for="selectAll">全选</label></div>';
    for (var i = 0; i < obj.length; i++) {
    	var infoFlag = false;
    	str += '<div class="commBody jyItem pay_sz subBox" id="subBox_'+obj[i].s_id+'" subId="'+obj[i].s_id+'">';//外层开始
    	str += '<div class="pay_sz_tit_box" style="display:none;>';
//    	str += ' <div class="in-checkbox check_box">';
    	str += ' <input style="margin-right:11px;margin-left: 6px;display:none;" type="checkbox" name="subCheck" id="subCheck_'+obj[i].s_id+'" onclick="subCheck(this,'+obj[i].s_id+')">';
//    	str += ' <label for="subCheck_'+obj[i].s_id+'">';
//    	str += ' 	<span>全选</span>';
//    	str += '  </label>';
//    	str += '</div>';
    
    	str += '  <div class=" lineP fr">';
    	str += '  	<span class="line_l" style="display:none;">已选择<font class="pay_yellow" id="ammeterCount_'+obj[i].s_id+'">0</font>个缴费号</span>';
    	str += '     <a class="line_l pay_btn" href="javascript:setupSub('+obj[i].s_id+');">开通手机缴费服务</a>	';
    	str += ' </div>';
...
//    	}
    	...
    	str += '<div class="jyTableBox1">';
    	str += '<table class="jyTable" cellpadding="0" cellspacing="0" border="0" width="100%" id="table_'+obj[i].s_id+'">';
    	str += '<tr><th width="33" class="frist_td"></th><th>序号</th><th>业务类型</th><th>缴费号</th><th>客户名称</th><th>应交金额(元)</th><th>缴费状态</th></tr>';//<th>操作</th>
    	var ammeter_type = "";
        var pay_statu = '';
        var bt_status = "";
		var num1 = 0;
        for (var j = 0; j < ammeters.length; j++) {
        	...
        	if(ammeters[j].proxy_flag==1){
        		str += '<td class="lineP frist_td">';
//        		str += '  <div class="in-checkbox">';
        		str += '   <input type="checkbox" name="ammeterCheck'+obj[i].s_id+'" id="ammeterCheck_'+ammeters[j].a_id+'" checked="checked" onclick="selone(this,'+obj[i].s_id+')">';
//        		str += '  <label for="ammeterCheck_'+ammeters[j].a_id+'" class="checked">';
//        		str += '  <span></span>';
//        		str += '  </label>';
//        		str += '  </div>';
        		str += '</td>';
        		...
        		str += '<td class="lineP frist_td">';
//        		str += '  <div class="in-checkbox">';
        		str += '   <input type="checkbox" name="ammeterCheck'+obj[i].s_id+'" id="ammeterCheck_'+ammeters[j].a_id+'" onclick="selone(this,'+obj[i].s_id+')">';
//        		str += '  <label for="ammeterCheck_'+ammeters[j].a_id+'">';
//        		str += '  <span></span>';
//        		str += '  </label>';
//        		str += '  </div>';
        		str += '</td>';
        	...
        	str += '<td>'+pay_statu+'</td>';
        	/*if (ammeters[j].pay_status == '0') {
        		str += '<td class="yjBtnTdBox"><a href="javascript:updateAmmeter('+eval(ammeters[j]).a_id+',1)" id="operat_'+eval(ammeters[j]).a_id+'">恢复</a><a href="javascript:bind2(\''+obj[i].s_id+'\');">详情</a><a href="javascript:delAmmeter(\''+eval(ammeters[j]).ammeter_no+'\')">删除</a>';
        	}
        	if (ammeters[j].pay_status == '1') {
        		str += '<td class="yjBtnTdBox"><a href="javascript:updateAmmeter('+eval(ammeters[j]).a_id+',0)" id="operat_'+eval(ammeters[j]).a_id+'">解绑</a><a href="javascript:bind2(\''+obj[i].s_id+'\');">详情</a><a href="javascript:delAmmeter(\''+eval(ammeters[j]).ammeter_no+'\')">删除</a>';
        	}
        	if(!ammeters[j].bill_img){
        		str += '<a href="javascript:upShowBillImg('+eval(ammeters[j]).a_id+')" id="upShowBillImg_'+eval(ammeters[j]).a_id+'"><label class="upFileLabel"><div id="file'+eval(ammeters[j]).a_id+'">上传缴费单</div></label></a>';
        	}else{
        		str += '<a href="javascript:showBillImg('+eval(ammeters[j]).a_id+')" id="showBillImg_'+eval(ammeters[j]).a_id+'">查看</a>';
        	}
        	str += '</td>'*/
        	str += '</tr>';
        }
//        str += '<tr><td></td><td>'+(++num1)+'</td><td>抄电表</td><td><div class="lines pItem" style="width:126px;"><input id="ammeterNum_bind_'+obj[i].s_id+'" type="text" placeholder="缴费号" class="pInput"></div></td><td></td><td></td><td></td><td class="yjBtnTdBox"><a href="javascript:bind(\''+obj[i].s_id+'\',\''+obj[i].c_id+'\');">绑定</a></td></tr>';
        str += '</table>';
        str += '</div>';
        //小计
        str += '<div class="lineP linePjyAllBox _lPay">'
        str += '	<span class="lines">应交金额小计：<font class="pay_yellow" id="sub_total_fee_'+obj[i].s_id+'">'+truncFee(eval(obj[i]).totalFee)+'</font> 元</span>';
...
function subCheck(o,subId){
	var count = 0;
	var xx = 0;
//	$(o).parent().parent().parent().find('.jyTableBox1').find('input[type=checkbox]').each(function(index,obj){
...
//	});
	if($(o).attr("checked")=="checked"){
		$("input[name='ammeterCheck"+subId+"']").each(function(){
			$(this).attr("checked","checked");
			count++;
		});
		var len = $("input[name^='ammeterCheck']").length;
		...
		if(xx==len){
			$("#selectAllChecked").attr("checked","checked");
		}
	}else{
		$("input[name='ammeterCheck"+subId+"']").each(function(){
		...
		$("#selectAllChecked").removeAttr("checked");
	}
	
	$("#ammeterCount_"+subId).html(count);
	updateAmmeterCount();
}
var upAid;
var upflag = true;
...
function upload_call_back1(response){
	if(response.code == '1'){
		...
		if(fileList.length == 1){
			...
			$("#upShowBillImg_"+upAid).html('查看');
			$("#upShowBillImg_"+upAid).attr('href','javascript:showBillImg('+upAid+')');
...
function setupSub(subId){
	var ids = "";
	$("#table_"+subId).find('input[type=checkbox]:checked').each(function(index,obj){
		ids+=$(obj).attr('id').replace('ammeterCheck_','')+',';
	})
	var url =  $("#projectPath").val() + '/public/withhold/setupSub';
	$.ajax({
        type: "POST",
        url: url,
        data: {
        ...
        success: function (data) {
            if (data.code == '1') {
            	ids = "";
            	$("#table_"+subId).find('input[type=checkbox]').each(function(index,obj){
            		if(!obj.checked){
            			ids +=$(obj).attr('id').replace('ammeterCheck_','')+',';
            		}
            	})
            	$.ajax({
            		...
            		success: function (data) {
            			if (data.code == '1') {
            				...
            				window.location.href= $("#projectPath").val() + '/public/withhold/submit';
            			} else {
...
function setupAll(){
	var ids = "";
	$('body').find('input[name^=ammeterCheck]:checked').each(function(index,obj){
		ids+=$(obj).attr('id').replace('ammeterCheck_','')+',';
	})
	var url =  $("#projectPath").val() + '/public/withhold/setupAll';
	$.ajax({
        type: "POST",
        url: url,
        data: {
...
function selone(obj,s_id){
	if($(obj).attr("checked")=="checked"){
		
	}else{
		$("#selectAllChecked").removeAttr("checked");
		$("#subCheck_"+s_id).removeAttr("checked");
	}
