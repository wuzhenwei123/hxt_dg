...
//    $("#paysum1").html(paysum);
    ...
    str += '<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="bk_xf">';
    str += '<tr>';
    ...
    str += ' <td valign="middle" align="center" class="heizi12"><strong>客户编号</strong></td>';
    ...
    for (var j = 0; j < ammeters.length; j++) {
        ...
        str += '<input type="text" style="text-align:center;cooler:#333333" maxlength="70" value="' + eval(ammeters[j]).ammeter_no + '" class="myInput ammeter_no" readonly="readonly" name="username[]">';
        str += '</span></td>';
        str += '<td width="10%" class="heizi12">' + eval(ammeters[j]).ammeter_name + '</td>';
        str += '<td width="10%" class="heizi12">' + totalFeeValue1 + '</td>';
        str += '<td align="right">';
        str += '<a class="lj_bc_14 detail_star_btn" href="javascript:void(0);" onclick="showDeatil(\''+eval(ammeters[j]).ammeter_no+'\',\''+ammeter_type+'\')">详情</a>';
        str += '</td>';
        str += ' </tr>';
        //5
    }
...
function showDeatil(ammeterNo,ammeterType){
...
function generatePayOrder(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 'success') {
                var url = $("#projectPath").val() + '/pay/generatePayOrder?cId='+cId;
                location.href = url;
            } else {
                alert(data.msg);
            }
