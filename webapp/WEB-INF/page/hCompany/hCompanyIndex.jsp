	                                ...
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                           ...
                                           <!--  <label class="margin5">fp_name:</label>
                                            ...
                                            <label class="margin5">推荐人手机号:</label>
	                                    	<input type="text" id="fRec_phone" class="span2 margin5" placeholder="推荐人手机号">
...
<!-- 	                                    	</select> -->
	                                    	...
	                                    	<c:if test="${agentFlag == true&&(not empty oneName ||not empty twoName)}"></c:if>
	                                    	<div class="widget-row form-inline row-fluid">
	                                   		<label class="margin5">客户经理:</label>
	                                    	<input type="text" id="agentOneNameQuery" readonly="readonly" onclick="modelType=1;showAgentOne()" data-toggle="modal" value="${oneName }">
									        ...
									        <input type="hidden" id="agentOneOpenIdQuery" value="${oneId }"/>
	                                   		<label class="margin5">代理:</label>
	                                    	<input type="text" id="agentTwoNameQuery" readonly="readonly" onclick="modelType=1;showAgentTwo()" data-toggle="modal" value="${twoName }">
									        ...
									        <input type="hidden" id="agentTwoOpenIdQuery" value="${twoId }"/>
	                                   		<label class="margin5">选择服务人员:</label>
	                                    	<input type="text" id="aServicerNameQuery" readonly="readonly" onclick="modelType=1;showServicer()" data-toggle="modal" value="${servicerName }">
									        ...
									        <input type="hidden" id="servicerIdQuery" value="${servicerId }"/>
	                                    </div>
	                                    </div>
	                                </div>
	                             ...
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	...
	                                 	 	<a href="javascript:exportUnPayHCompanyExcel(2);" class="btn btn-glyph">导出本月没有制单的单位</a>
	                                 	 	<a href="javascript:exportUnPayHCompanyExcel(1);" class="btn btn-glyph">导出本月制单没有缴费的单位</a>
	                                 	 	<a href="javascript:exportPayHCompanyExcel();" class="btn btn-glyph">导出本月有缴费之前未缴费单位</a>
...
var modelType = 1;//代理查询弹出框类型 1 查询 2 新增
var $stack3 = $('#stack3');
var $stack2 = $('#stack2');
var $stack4 = $('#stack4');
...
function cleanOn4(){
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    var rec_phone = $("#fRec_phone").val();
		    var agentOneOpenId = $("#agentOneOpenIdQuery").val();
		    var agentTwoOpenId = $("#agentTwoOpenIdQuery").val();
		    var servicerId = $("#servicerIdQuery").val();
		    var create_time1 = $("#fCreate_time1").val();
		    var create_time2 = $("#fCreate_time2").val();
		    var rateFlag = $("#fRateFlag").val();
		    $.getJSON("<c:url value='/hCompany/getHCompanyList'/>",
	        {
		    	startTime:create_time1,
		    	endTime:create_time2,
...
// 	    	str+= "<th class=\"sortTh\" id=\"th_user_id\" column='user_id' onselectstart='return false' scope=\"col\">user_id</th>";
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_ywyId\" column='ywyId' onselectstart='return false' scope=\"col\">推荐人手机号</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ywyId\" column='ywyId' onselectstart='return false' scope=\"col\">客户经理</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ywyId\" column='ywyId' onselectstart='return false' scope=\"col\">代理</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ywyId\" column='ywyId' onselectstart='return false' scope=\"col\">服务人员</th>";
...
// 		        tableStr += "<td>" + result.items[k].user_id + "</td>";
		      ...
		      /*   tableStr += "<td>" + result.items[k].fp_name + "</td>";
		        ...
		        tableStr += "<td>" + result.items[k].rec_phone + "</td>";
...
/* 		        tableStr += "<td>" + result.items[k].remark1 + "</td>";
        	...
        	if(verify_status==0){
	        	tableStr += "<perm:tag permPath='/hCompany/toInfo' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toInfo(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />审核</a></perm:tag>";
        	}
        	var _status = result.items[k].status;
        	if(_status==1){
	        	tableStr += "<perm:tag permPath='/hCompany/verify' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toVerify(" + result.items[k].id + ",2);return false;'><i class='fontello-icon-edit' />暂停</a></perm:tag>";
        	}else if(_status==2){
	        	tableStr += "<perm:tag permPath='/hCompany/verify' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toVerify(" + result.items[k].id + ",1);return false;'><i class='fontello-icon-edit' />恢复</a></perm:tag>";
        	}
        	if(result.items[k].rateFlag==1){
	        	tableStr += "<perm:tag permPath='/hCompany/israteFlag' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='israteFlag(" + result.items[k].id + ",0);return false;'><i class='fontello-icon-edit' />取消分润</a></perm:tag>";
        	}else if(result.items[k].rateFlag==0){
	        	tableStr += "<perm:tag permPath='/hCompany/israteFlag' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='israteFlag(" + result.items[k].id + ",1);return false;'><i class='fontello-icon-edit' />恢复分润</a></perm:tag>";
        	}
        	tableStr += "<perm:tag permPath='/hCompany/updateHCompany' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hCompany/removeHCompany' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "<a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='showDetail(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />查看详情</a>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function israteFlag(id,type){
		if (id != ""){
			...
			var html = "";
			   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要"+verStr+"吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hCompany/israteFlag'/>",
		        	{
						id	:id,
						type:type,
						ranNum:Math.random()
...
//     		create_time : create_time,
	...
	function exportUnPayHCompanyExcel(exportType){
		var returnNum = $("#returnNum").val();
		var sortColumn = $("#sortColumn").val();
	    var id = $("#fId").val();
	    var name = $("#fName").val();
	    ...
	    var remark6 = $("#fRemark6").val();
	    $(".loading-box").show();
	    $.getJSON("<c:url value='/hCompany/exportUnPayHCompanyExcel'/>",
        {
        	sortColumn:sortColumn,
    		id : id,
    		name : name,
	...
	function exportPayHCompanyExcel(){
		var returnNum = $("#returnNum").val();
		var sortColumn = $("#sortColumn").val();
	    var id = $("#fId").val();
	    var name = $("#fName").val();
	    ...
	    var remark5 = $("#fRemark5").val();
	    var remark6 = $("#fRemark6").val();
	    $.getJSON("<c:url value='/hCompany/exportPayHCompanyExcel'/>",
        {
        	sortColumn:sortColumn,
    		id : id,
    		name : name,
	...
	function showDetail(id){
		show('<c:url value="/hCompany/showDetail/'+id+'"/>','');
	}
</script>
</body>
</html>
