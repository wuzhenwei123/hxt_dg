...
<script type="text/javascript">
	...
	function setTableStr2(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        if(result.items[k].id==gl_yl_id){
	        	tableStr += "<td style='float:right;'><input type=\"radio\" checked value='"+result.items[k].name+"' id='"+result.items[k].id+"' class=\"checkbox check-row\" name=\"sjOneRadio\" /></td>";
	        }else{
	...
	function agentOneSure(id,glylname){
		$.post("<c:url value='/hAgent/setGlYL'/>",
	        	{
					glylid	:id,
					id	:agentId,
					glylname	:glylname,
