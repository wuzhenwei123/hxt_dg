...
<!-- 	                                 <div class="btn-toolbar">  -->
<!-- 	                                 	 <div class="btn-group">  -->
<!-- 	                                 	 	<perm:tag permPath="/hProxyMessageLog/toAdd" > -->
<!-- 	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a>  -->
<!-- 	                                 	 	</perm:tag> -->
<!-- 	                                 	 	<perm:tag permPath="/hProxyMessageLog/removeAllHProxyMessage" > -->
...
<!-- <div class="loading-box maskbg"> -->
	<img src="${ctx}/images/loading.gif" id="dddd" width="120px" height="120px"/>
<!-- </div> -->
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    $.getJSON("<c:url value='/hProxyMessageLog/getHProxyMessageLogList'/>",
	        {
	...
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
        	...
        	if(result.items[k].hetongUrl!=""){
        		tableStr += "<a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='preview(this,\"" + result.items[k].hetongUrl + "\");return false;'><i class='fontello-icon-eye-1' />查看开通协议</a>";
        	}
        	if(result.items[k].chexiaoUrl!=""){
        		tableStr += "<a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='preview(this,\"" + result.items[k].chexiaoUrl + "\");return false;'><i class='fontello-icon-eye-1' />查看终止协议</a>";
        	}
        	if(result.items[k].biangengUrl!=""){
        		tableStr += "<a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='preview(this,\"" + result.items[k].biangengUrl + "\");return false;'><i class='fontello-icon-eye-1' />查看变更协议</a>";
        	}
        	tableStr += "</div>";
        	tableStr += "</td>";
...
// 		        tableStr += "<td>" + result.items[k].payName + "</td>";
	...
	function toUpdate(id) {
		show('<c:url value="/hProxyMessageLog/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hProxyMessageLog/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$(".loading-box").show();
					$.post("<c:url value='/hProxyMessageLog/removeAllHProxyMessage'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$(".loading-box").show();
				$.post("<c:url value='/hProxyMessageLog/removeHProxyMessage'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
	...
	function shenhe(id){
		show('<c:url value="/hProxyMessageLog/toShenhe/'+id+'"/>','');
// 	   if (id != ""){
// 		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要审核吗?","取消","确定", function(result) {
// 			if(result){
// 				$(".loading-box").show();
// 				$.post("<c:url value='/hProxyMessageLog/shenhe'/>",
// 	        	{
// 					id	:id,
// 					cId	:id,
// 					ranNum:Math.random()
...
// 	    $("#demo4").find("radio").uniform();//初始化复选框
	...
	function zzshenhe(id){
		show('<c:url value="/hProxyMessageLog/tozzShenhe/'+id+'"/>','');
	}
	
	function bgshenhe(id){
		show('<c:url value="/hProxyMessageLog/tobgShenhe/'+id+'"/>','');
	}
	
	function preview(obj,hetongUrl){
		//获取图片的宽和高
	    ...
	    image.onload = function(){
	    	var naturalWidth = image.width;
	 		...
	 		$(".preview").find("img").attr('src',$("#pic_front").val()+hetongUrl);
	...
	function closePreview(){
		$(".preview").hide();
		$(".preview").find("img").attr('src',"${ctx}/images/loading.gif");
		$(".preview").find("img").attr('width','32');
 		$(".preview").find("img").attr('height','32');
	}
</script>
</body>
</html>
