	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hPersonType/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
<%-- 	                                 	 	<perm:tag permPath="/hPersonType/removeAllHPersonType" > --%>
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    $.getJSON("<c:url value='/hPersonType/getHPersonTypeList'/>",
	        {
	...
	function genTableHeader(){
		var str = "<thead><tr>" ;
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_adminId\" column='adminId' onselectstart='return false' scope=\"col\">创建人</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_lastTime\" width='135' column='lastTime' onselectstart='return false' scope=\"col\">最后更改时间<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_lastAdminId\" column='lastAdminId' onselectstart='return false' scope=\"col\">最后更改人</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_remark\" column='remark' onselectstart='return false' scope=\"col\">备注</th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	}
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
        	...
        	tableStr += "<perm:tag permPath='/hPersonType/updateHPersonType' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hPersonType/removeHPersonType' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hPersonType/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hPersonType/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hPersonType/removeAllHPersonType'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hPersonType/removeHPersonType'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
