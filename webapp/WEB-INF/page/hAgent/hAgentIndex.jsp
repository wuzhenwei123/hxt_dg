...
// 		        tableStr += "<td>" + result.items[k].remark3 + "</td>";
        	...
        	tableStr += "<perm:tag permPath='/hAgent/updateHAgent' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hAgent/removeHAgent' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	if(result.items[k].check_status==1){
	        	tableStr += "<perm:tag permPath='/hAgent/qrcode' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='qrcode(" + result.items[k].id+","+result.items[k].check_status + ");return false;'><i class=' fontello-icon-picture' />二维码</a></perm:tag>";
        	}
        	if(result.items[k].check_status!=1){
	        	tableStr += "<perm:tag permPath='/hAgent/review' ><a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='review(" + result.items[k].id + ");return false;'><i class='fontello-icon-eye-1' />审核</a></perm:tag>";
        	}
        	if(result.items[k].remark3!='1'){
	        	tableStr += "<perm:tag permPath='/hAgent/isDefault' ><a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='isDefault(" + result.items[k].id + ");return false;'><i class='fontello-icon-eye-1' />设为默认</a></perm:tag>";
        	}
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
