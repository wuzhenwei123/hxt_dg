...
<body class="sidebar-left">
	...
	<input type="hidden" id="sortColumn" value="a.sortId desc"><!-- 排序字段 -->
	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                            <label class="margin5">标题:</label>
	                                    	<input type="text" id="fTitle" class="span2 margin5" placeholder="标题">
                                            <label class="margin5">内容:</label>
	                                    	<input type="text" id="fContent" class="span2 margin5" placeholder="内容">
<!--                                             <label class="margin5">创建时间:</label> -->
<!-- 	                                    	<input type="text" id="fCreateTime" placeholder="创建时间" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="span2 margin5" size="15" readonly="readonly"> -->
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hNewNotice/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hNewNotice/removeAllHnewNotice" >
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    $.getJSON("<c:url value='/hNewNotice/getHNewNoticeList'/>",
	        {
...
// 	    	str+= "<th class=\"sortTh\" id=\"th_id\" column='id' onselectstart='return false' scope=\"col\">id<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_title\" column='title' onselectstart='return false' scope=\"col\">标题<span class=\"pull-right aweso-icon-sort\"></span></th>";
// 	    	str+= "<th class=\"sortTh\" id=\"th_content\" column='content' onselectstart='return false' scope=\"col\">内容<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_createTime\" width='135' column='createTime' onselectstart='return false' scope=\"col\">创建时间<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_sortId\" column='sortId' onselectstart='return false' scope=\"col\">排序<span class=\"pull-right aweso-icon-sort\"></span></th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	}
...
// 		        tableStr += "<td>" + result.items[k].content + "</td>";
		        ...
		        tableStr += "<td>" + result.items[k].sortId + "</td>";
	        tableStr += "<td>";
	        tableStr += "<div class=\"btn-group\">";
        	tableStr += "<perm:tag permPath='/hNewNotice/updateHNewNotice' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hNewNotice/removeHNewNotice' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hNewNotice/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hNewNotice/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hNewNotice/removeAllHNewNotice'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hNewNotice/removeHNewNotice'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
