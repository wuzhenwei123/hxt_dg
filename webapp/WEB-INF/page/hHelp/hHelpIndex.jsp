...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    $.getJSON("<c:url value='/hHelp/getHHelpList'/>",
	        {
...
// 		        tableStr += "<td>" + (result.items[k].state==1?"正常":"禁用") + "</td>";
        	...
        	tableStr += "<perm:tag permPath='/hHelp/updateHHelp' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hHelp/removeHHelp' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toShow(id){
		show('<c:url value="/hHelp/toShow/'+id+'"/>','');
	}
	// 更新
	function toUpdate(id) {
		show('<c:url value="/hHelp/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hHelp/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hHelp/removeAllHHelp'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hHelp/removeHHelp'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
