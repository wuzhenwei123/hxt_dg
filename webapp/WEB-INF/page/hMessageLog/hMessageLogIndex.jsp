	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
	                                    	...
	                                    	<input type="text" id="fPhone" class="span2 margin5" placeholder="手机号">
                                            <label class="margin5">发送内容:</label>
	                                    	<input type="text" id="fContent" class="span2 margin5" placeholder="发送内容">
                                            <label class="margin5">IP:</label>
	                                    	<input type="text" id="fIp" class="span2 margin5" placeholder="IP">
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hMessageLog/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hMessageLog/removeAllHMessageLog" >
...
// 		    var createTime = $("#fCreateTime").val();
		    $.getJSON("<c:url value='/hMessageLog/getHMessageLogList'/>",
	        {
...
// 	    	str+= "<th class=\"sortTh\" id=\"th_id\" column='id' onselectstart='return false' scope=\"col\">id<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_phone\" column='phone' onselectstart='return false' scope=\"col\">手机号<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_content\" column='content' onselectstart='return false' scope=\"col\">发送内容<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ip\" column='ip' onselectstart='return false' scope=\"col\">IP<span class=\"pull-right aweso-icon-sort\"></span></th>";
...
// 		        tableStr += "<td>" + result.items[k].id + "</td>";
        	...
        	tableStr += "<perm:tag permPath='/hMessageLog/updateHMessageLog' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hMessageLog/removeHMessageLog' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hMessageLog/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hMessageLog/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hMessageLog/removeAllHMessageLog'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hMessageLog/removeHMessageLog'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
