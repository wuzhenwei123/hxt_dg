...
<script type="text/javascript">
	...
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
        	...
        	tableStr += "<perm:tag permPath='/hAgentTwo/updateHAgentTwo' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hAgentTwo/removeHAgentTwo' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	if(result.items[k].check_status==1){
	        	tableStr += "<perm:tag permPath='/hAgentTwo/qrcode' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='qrcode(" + result.items[k].id+","+result.items[k].check_status + ");return false;'><i class=' fontello-icon-picture' />二维码</a></perm:tag>";
        	}
        	if(result.items[k].check_status!=1){
	        	tableStr += "<perm:tag permPath='/hAgentTwo/review' ><a class='btn btn-red btn-mini no-wrap' href='javascript:void(0);' onclick='review(" + result.items[k].id + ");return false;'><i class='fontello-icon-eye-1' />审核</a></perm:tag>";
        	}
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
