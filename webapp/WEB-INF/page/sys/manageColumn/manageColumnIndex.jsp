	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                            ...
                                            <label class="margin5">父菜单：</label>
                                            <select id="fParentColumnID" class="span2 margin5"> 
                                            ...
                                            <label class="margin5">菜单ID：</label>
                                            <input type="text" id="fColumnId" class="span2 margin5" placeholder="菜单ID">
                                            <label class="margin5">菜单名称：</label>
	                                    	<input type="text" class="span2 margin5" id="fColumnName" placeholder="菜单名称">
...
<script type="text/javascript">
	...
	function genTableHeader(){
		var str = "<thead><tr>" ;
	    	...
	    	str+= "<th width='30' onselectstart='return false' scope=\"col\">序号</th>";
	    	str+= "<th width='50' class=\"sortTh\" id=\"th_columnId\" column='columnId' onselectstart='return false' scope=\"col\">菜单ID<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th id=\"th_columnName\" column='columnName' onselectstart='return false' scope=\"col\">菜单名称<span class=\"pull-right\"></span></th>";
	    	str+= "<th id=\"th_columnUrl\" column='columnUrl' onselectstart='return false' scope=\"col\">菜单路径<span class=\"pull-right\"></span></th>";
	    	str+= "<th id=\"th_parentColumnID\" column='parentColumnID' onselectstart='return false' scope=\"col\">父菜单<span class=\"pull-right\"></span></th>";
	    	str+= "<th width='30' onselectstart='return false' scope=\"col\">图标<span class=\"pull-right\"></span></th>";
	    	str+= "<th width='50' class=\"sortTh\" id=\"th_state\" column='state' onselectstart='return false' scope=\"col\">状态<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    	str+= "<th width='50' id=\"th_columnOrder\" column='columnOrder' onselectstart='return false' scope=\"col\">排序<span class=\"pull-right\"></span></th>";
//		    	str+= "<th >菜单图片</th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	...
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        tableStr += "<td ><input type=\"checkbox\" id='"+result.items[k].columnId+"' class=\"checkbox check-row\" value=\"0\" name=\"checkName\" /></td>";
	        ...
	        if(columnIm == ''){
	        	columnIm = "fontello-icon-monitor";
	        }
	        
	        tableStr += "<td ><i class='" + columnIm +"'></i></td>";
	        tableStr += "<td >" + (result.items[k].state==1?"正常":"禁用") + "</td>";
	        tableStr += "<td ><a class='btn btn-mini' onclick=\"move('"+result.items[k].parentColumnID+"','" + result.items[k].columnId + "','up')\" ><i title=\"上移动\" class='aweso-icon-caret-up' /></a><a class='btn btn-mini' onclick=\"move('"+result.items[k].parentColumnID+"','" + result.items[k].columnId + "','down')\" ><i title=\"下移动\" class='aweso-icon-caret-down' /></a></td>"; 
	        tableStr += "<td>";
	        tableStr += "<div class=\"btn-group\">";
        	tableStr += "<perm:tag permPath='/manageColumn/toUpdate' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].columnId + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/manageColumn/removeManageColumn' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].columnId + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/manageColumn/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/manageColumn/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
