...
<div id="stack2" class="modal hide fade" tabindex="-1" data-focus-on="input:first" data-width="80%">
    ...
    <div class="modal-footer"> 
	     ...
	     <button type="submit" class="btn btn-green" onclick="saveyl()">保存</button>
    </div>
</div>
...
<script type="text/javascript">
	...
	function setTableStr2(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        if(result.items[k].id==gl_yl_id){
		        tableStr += "<td style='float:right;'><input type=\"radio\" checked value='"+result.items[k].name+"' id='"+result.items[k].id+"' class=\"checkbox check-row\" value=\"0\" name=\"sjOneRadio\" /></td>";
	        }else{
		        tableStr += "<td style='float:right;'><input type=\"radio\" value='"+result.items[k].name+"' id='"+result.items[k].id+"' class=\"checkbox check-row\" value=\"0\" name=\"sjOneRadio\" /></td>";
	        }
	...
	function agentTwoSure(id,glylname){
		$.post("<c:url value='/manageAdminUser/setGlYL'/>",
	        	{
					glylid	:id,
					id	:userId,
					glylname	:glylname,
