	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group">
	                                 	 	<c:if test="${hfuwuListCount ==0 }">
	                                 	 	<perm:tag permPath="/hFuwu/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	</c:if>
	                                 	 	<perm:tag permPath="/hFuwu/removeAllHFuwu" >
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    $.getJSON("<c:url value='/hFuwu/getHFuwuList'/>",
	        {
...
// 		        tableStr += "<td>" + (result.items[k].state==1?"正常":"禁用") + "</td>";
        	...
        	tableStr += "<perm:tag permPath='/hFuwu/updateHFuwu' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hFuwu/removeHFuwu' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toShow(id){
		show('<c:url value="/hFuwu/toShow/'+id+'"/>','');
	}
	// 更新
	function toUpdate(id) {
		show('<c:url value="/hFuwu/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hFuwu/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hFuwu/removeAllHFuwu'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hFuwu/removeHFuwu'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
