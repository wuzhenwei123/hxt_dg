	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
	                                    	...
	                                    	<input type="text" id="fAmmeter_no" class="span2 margin5">
	                                    	...
	                                    	<input type="text" id="fRec_phone" class="span2 margin5">
                                            <label class="margin5">客户联系人手机号:</label>
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hRecRecord/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hRecRecord/removeAllHRecRecord" >
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    var ammeterinfo_type = $("#fAmmeterinfo_type").val();
		    var contact_phone = $("#fContact_phone").val();
		    var rec_phone = $("#fRec_phone").val();
		    $.getJSON("<c:url value='/hRecRecord/getHRecRecordList'/>",
	        {
	...
	function genTableHeader(){
		var str = "<thead><tr>" ;
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_ammeter_no\" column='ammeter_no' onselectstart='return false' scope=\"col\">客户联系人手机号</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ammeter_name\" column='ammeter_name' onselectstart='return false' scope=\"col\">客户名称</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_create_time\" width='135' column='create_time' onselectstart='return false' scope=\"col\">记录时间</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_ammeter_address\" column='ammeter_address' onselectstart='return false' scope=\"col\">客户地址</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_rec_phone\" column='rec_phone' onselectstart='return false' scope=\"col\">推荐人手机号</th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	}
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
        	...
        	tableStr += "<perm:tag permPath='/hRecRecord/updateHRecRecord' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hRecRecord/removeHRecRecord' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hRecRecord/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hRecRecord/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hRecRecord/removeAllHRecRecord'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hRecRecord/removeHRecRecord'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
