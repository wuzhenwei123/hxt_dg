...
<script type="text/javascript">
	...
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
        	...
        	tableStr += "<perm:tag permPath='/hPayGuli/updateHPayGuli1' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hPayGuli/removeHPayGuli1' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hPayGuli/toUpdate1/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hPayGuli/toAdd1"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
