	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                            <label class="margin5">登录名:</label>
	                                    	<input type="text" id="fAdminName" class="span2 margin5">
                                            <label class="margin5">登录IP:</label>
	                                    	<input type="text" id="fLoginIp" class="span2 margin5">
                                            <label class="margin5">登录设备类型:</label>
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hLoginLog/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hLoginLog/removeAllHLoginLog" >
	                                 	 	<a href="javascript:delAll();" class="btn btn-glyph"><i class="fontello-icon-minus-1"></i>删除</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hLoginLog/exportExcel" >
	                                 	 	<a href="javascript:exportExcel();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>导出</a> 
	                                 	 	</perm:tag>
	                                 	</div>
	                                 </div>
	                             </div>
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    var loginTIme = $("#fLoginTIme").val();
		    var loginIp = $("#fLoginIp").val();
		    var deviceType = $("#fDeviceType").val();
		    var remark1 = $("#fRemark1").val();
		    var remark2 = $("#fRemark2").val();
		    var remark3 = $("#fRemark3").val();
		    $.getJSON("<c:url value='/hLoginLog/getHLoginLogList'/>",
	        {
	...
	function genTableHeader(){
		var str = "<thead><tr>" ;
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_adminName\" column='adminName' onselectstart='return false' scope=\"col\">登录账户</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_loginTIme\" width='135' column='loginTIme' onselectstart='return false' scope=\"col\">最后一次登录时间</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_loginIp\" column='loginIp' onselectstart='return false' scope=\"col\">最后一次登录IP</th>";
	    	str+= "<th class=\"sortTh\" id=\"th_deviceType\" column='deviceType' onselectstart='return false' scope=\"col\">登录设备类型</th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	}
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        tableStr += "<td >" + (number + k + 1) + "</td>";
		        ...
		        tableStr += "<td>" + genDateTimeAll(result.items[k].loginTIme) + "</td>";
		        tableStr += "<td>" + result.items[k].loginIp + "</td>";
		        var deviceType = result.items[k].deviceType;
		        if(deviceType=="0"){
		        	tableStr += "<td>PC</td>";
		        }else{
		        	tableStr += "<td>微信</td>";
        	...
        	tableStr += "<perm:tag permPath='/hLoginLog/updateHLoginLog' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hLoginLog/removeHLoginLog' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hLoginLog/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hLoginLog/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hLoginLog/removeAllHLoginLog'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hLoginLog/removeHLoginLog'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
	...
	function exportExcel(){
	    ...
	    var remark2 = $("#fRemark2").val();
	    var remark3 = $("#fRemark3").val();
	    $.getJSON("<c:url value='/hLoginLog/exportExcel'/>",
        {
        	sortColumn:sortColumn,
    		id : id,
    		adminName : adminName,
