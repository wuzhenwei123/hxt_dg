	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                            ...
                                            <label class="margin5">provinceName:</label>
	                                    	<input type="text" id="fProvinceName" class="span2 margin5" placeholder="provinceName">
	                                    </div>
	                                </div>
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/hProvince/toAdd" >
	                                 	 	<a href="javascript:toAdd();" class="btn btn-glyph"><i class="fontello-icon-plus-1"></i>新增</a> 
	                                 	 	</perm:tag>
	                                 	 	<perm:tag permPath="/hProvince/removeAllHProvince" >
...
<script type="text/javascript">
	...
	function searchData(pageNo){
		var returnNum = $("#returnNum").val();
		    ...
		    var provinceName = $("#fProvinceName").val();
		    $.getJSON("<c:url value='/hProvince/getHProvinceList'/>",
	        {
	...
	function genTableHeader(){
		var str = "<thead><tr>" ;
	    	...
	    	str+= "<th class=\"sortTh\" id=\"th_provinceName\" column='provinceName' onselectstart='return false' scope=\"col\">provinceName<span class=\"pull-right aweso-icon-sort\"></span></th>";
			str+="<th>操	作</th>";
			str+="</tr></thead>";
		return str;
	}
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        tableStr += "<td >" + (number + k + 1) + "</td>";
		        ...
		        tableStr += "<td>" + result.items[k].provinceName + "</td>";
	        tableStr += "<td>";
	        tableStr += "<div class=\"btn-group\">";
        	tableStr += "<perm:tag permPath='/hProvince/updateHProvince' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].id + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/hProvince/removeHProvince' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].id + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/hProvince/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/hProvince/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
		...
		if(ids!=''){
			bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
				if(result){
					$.post("<c:url value='/hProvince/removeAllHProvince'/>",
		        	{
						ids :ids,
						ranNum:Math.random()
					},
	...
	function del(id){
	   if (id != ""){
		   bootbox.confirm("<i class=' fontello-icon-emo-happy' />你确定要删除吗?","取消","确定", function(result) {
			if(result){
				$.post("<c:url value='/hProvince/removeHProvince'/>",
	        	{
					id	:id,
					ranNum:Math.random()
				},
