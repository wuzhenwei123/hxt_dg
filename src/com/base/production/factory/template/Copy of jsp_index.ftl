...
<body class="${jl}system_skins_navgSet${jld} ">
	                            ...
	                            <div class="widget-content collapse" id="demoB" style="height: 0px;">
	                                <div class="widget-body">
	                                    <div class="widget-row form-inline row-fluid">
                                            ...
                                            <label class="margin5">${item.name}:</label>
	                                    	<#if item.type == "Date">
	                                    	<input type="text" id="f${item.name?cap_first}" placeholder="${item.name}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="span2 margin5" size="15" readonly="readonly">
	                                    	<#else>
	                                    	<#if item.name == "state">
	                                    	...
	                                    	<#else>
	                                    	<input type="text" id="f${item.name?cap_first}" class="span2 margin5" placeholder="${item.name}">
	                                    	</#if>
	                                    	</#if>
	                                    	</#list>
	                                    </div>
	                    ...
	                    <div class="row-fluid">
	                    	<div class="span12 widget widget-simple bg-gray-light">
	                             <div class="widget-header">
	                                 <div class="btn-toolbar"> 
	                                 	 <div class="btn-group"> 
	                                 	 	<perm:tag permPath="/${domainName}/toAdd" >
...
<script type="text/javascript">
	...
	function genTableHeader(){
		...
		<#list columns as item>
		<#if item.type == "Date">
	    	str+= "<th class=\"sortTh\" id=\"th_${item.name}\" width='135' column='${item.name}' onselectstart='return false' scope=\"col\">${item.name}<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    <#else>
	    	str+= "<th class=\"sortTh\" id=\"th_${item.name}\" column='${item.name}' onselectstart='return false' scope=\"col\">${item.name}<span class=\"pull-right aweso-icon-sort\"></span></th>";
	    </#if>
		</#list>	
			str+="<th>操	作</th>";
			str+="</tr></thead>";
	...
	function setTableStr(result, pageNo, returnNum){
	    ...
	    for (var k=0; k<result.items.length; k++){      
	        ...
	        tableStr += "<td ><input type=\"checkbox\" id='"+result.items[k].${pkName}+"' class=\"checkbox check-row\" value=\"0\" name=\"checkName\" /></td>";
        	...
        	tableStr += "<perm:tag permPath='/${domainName}/update${domainName?cap_first}' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='toUpdate(" + result.items[k].${pkName} + ");return false;'><i class='fontello-icon-edit' />编辑</a></perm:tag>";
        	tableStr += "<perm:tag permPath='/${domainName}/remove${domainName?cap_first}' ><a class='btn btn-yellow btn-mini no-wrap' href='javascript:void(0);' onclick='del(" + result.items[k].${pkName} + ");return false;'><i class='fontello-icon-minus-1' />删除</a></perm:tag>";
        	tableStr += "</div>";
        	tableStr += "</td>";
	        tableStr += "</tr>";            
	    }
	...
	function toUpdate(id) {
		show('<c:url value="/${domainName}/toUpdate/'+id+'"/>','');
	}
		
	// 添加
	function toAdd(){
		show('<c:url value="/${domainName}/toAdd"/>','');
	}
	// 删除所选
	function delAll(){
		var ids = '';
